...
class UdpComms():
    def __init__(self,udpIP,pcIP,portTX,portRX,enableRX=False,suppressWarnings=True):
        self.pcIP = pcIP
        self.udpIP = udpIP
        ...
        print(udpIP) # allows the address/port to be reused immediately instead of it being stuck in the TIME_WAIT state waiting for late packets to arrive.
    ...
    def SendData(self, strToSend):
        ...
        print((self.pcIP, self.udpSendPort))
        self.pcSock.sendto(strToSend.encode('utf-8'), (self.pcIP, self.udpSendPort))
